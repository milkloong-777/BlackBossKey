...
**常驻状态灯**：每块屏右上角 28×28 小圆点，`TransparencyKey = Color.Fuchsia` 抠透明背景，`OnPaint` 按开关状态画不同颜色——**红 = 黑幕开启，绿 = 关闭**。它是**常驻窗口**，不随黑屏清理销毁，只在开关时变色（`SetOn` → `Invalidate` 重绘）。**不做捕获排除**，所以手机远程画面也能看到，随时分辨开关状态。同样点击穿透，同样参与 250ms 置顶重申；显示器布局变化时 `EnsureLights` 跟随重建。
