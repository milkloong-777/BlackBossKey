...
**状态灯**：每块屏右上角 28×28 小圆点，`TransparencyKey = Color.Fuchsia` 抠透明背景，`OnPaint` 按开关状态画不同颜色——**红 = 黑幕开启（常驻），绿 = 关闭（5 秒后淡出）**。淡出用窗体 `Opacity` 属性逐帧降低实现（200ms 定时器，前 5 秒保持全亮、之后每 200ms 降 0.1 约 1 秒淡完，然后 `Hide()`）。红/绿状态切换用 `SetOn` → `Invalidate` 重绘；**不做捕获排除**，所以手机远程画面也能看到。同样点击穿透，同样参与 250ms 置顶重申；显示器布局变化时 `EnsureLights` 跟随重建。
